...
;(function ($, window, document, undefined) {
    ...
    var defaults = {
        ...
        resultName: 'order_list', // Submit name of the cart parameter
        theme: 'default', // theme for the cart, related css need to include for other than default theme
        combineProducts: true, // combine similar products on cart
        highlightEffect: true, // highlight effect on adding/updating product in cart
        cartItemTemplate: '<img class="img-responsive pull-left" src="{item_image}" /><h5 class="list-group-item-heading title">{item_name}</h5>',
    ...
    $.extend(SmartCart.prototype, {
        ...
        _setElements: function () {
            ...
            this.cart_element.append('<div class="panel-heading sc-cart-heading text-bold">' + this.options.lang.cartTitle + ' <span class="sc-cart-count badge">0</span></div>');
            this.cart_element.append('<table id="ItemList" class="table table-hover sc-cart-item-list"></table>');
          
            $( '<thead><tr><th scope="col">Item Name</th><th scope="col">Price</th></tr></thead><tbody></tbody>' ).appendTo( ".sc-cart-item-list" );
        },
        /* 
        ...
        _setToolbar: function () {
            ...
            if (this.options.toolbarSettings.showCheckoutButton) {
                ...
                switch (this.options.toolbarSettings.checkoutButtonStyle) {
                    ...
                    case 'image':
                        ...
                        break;
                    default:
                        btnCheckout = '<button class="btn btn-success sc-cart-checkout pull-right" type="button">' + this.options.lang.checkout + '</button> ';
                        break;
                }
            ...
            if (this.options.toolbarSettings.showCartSummary) {
                var panelSubTotal = $('<div class="sc-cart-summary-subtotal text-bold">');
        ...
        _addUpdateCartItem: function (p) {
            ...
            if (elmMain && elmMain.length > 0) {
                ...
                elmMain = $('<tr></tr>').addClass('sc-cart-item '); // list-group-item
               elmMain.attr('data-unique-key', p.unique_key);

                elmMain.append(this._formatTemplate(this.options.cartItemTemplate, p));
                elmMain.append('<button type="button" class="sc-cart-remove pull-right">' + this.options.lang.cartRemove + '</button>');
                ...
                itemSummary += ' = <span class="sc-cart-item-amount">' + this._getMoneyFormatted(productAmount) + '</span></div> <hr></hr>';
